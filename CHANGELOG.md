...
- **`mockProps`** — résout le cold start. Le composant est rendu invisiblement avec ces props fictives dès le premier chargement, permettant au fiber walker de mesurer un layout réaliste avant que les vraies données arrivent. Dès que le vrai layout est capturé, `mockProps` n'est plus jamais utilisé.
